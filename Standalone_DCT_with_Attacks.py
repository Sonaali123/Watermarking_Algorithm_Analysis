...
Original file is located at
    https://colab.research.google.com/drive/17H-hNUFh2Y6WYxSOmLNM9obKrG5Deb_n
"""
...
def embed_watermark_dct(cover_pil, watermark_text, alpha=10):
    """Embed a text watermark into the image using DCT."""
    ...
    # Embed bits into middle frequency coefficients
    i, j = 0, 0
    for bit in watermark_bin:
        # Avoid DC (0,0), modify mid-frequency
        dct[i+10, j+10] += alpha if bit == 1 else -alpha
        j += 1
...
def extract_watermark_dct(cover_pil, watermarked_pil, alpha=10, bit_length=None):
    """Extract the embedded text watermark from the watermarked image using DCT."""
    ...
    for _ in range(bit_length):
        diff = dct_watermarked[i+10, j+10] - dct_cover[i+10, j+10]
...
for fname in os.listdir(dataset_path):
    ...
    # Embed watermark
    watermarked_pil, watermark_bin = embed_watermark_dct(img_pil, "CopyRightVIT", alpha=15)
    ...
    for atk_name, atk_img in attacks.items():
        ...
        extracted_pil, extracted_bits = extract_watermark_dct(img_pil, atk_pil, alpha=15, bit_length=len(watermark_bin))
...
df.to_csv("/content/dct_results.csv", index=False)
df.head()